...
bool CModuleSceneManager::loadScene(const std::string & name) {
  PROFILE_FUNCTION("Load Scene");
  //CTraceScoped gpu_scope("Load Scene");
  ...
  if (it != _scenes.end())
  {
    ...
    {
      PROFILE_FUNCTION("Delete Checkpoints");
    ...
    {
      PROFILE_FUNCTION("Unload active scene");
      unLoadActiveScene();
    }
    {
      // Load the subscene
      PROFILE_FUNCTION("Loading subscenes");
      Scene * current_scene = it->second;
      Engine.getNavmeshes().buildNavmesh(current_scene->navmesh);


    for (auto& scene_name : current_scene->groups_subscenes) {
      PROFILE_FUNCTION("Subscene %s", scene_name.c_str());
