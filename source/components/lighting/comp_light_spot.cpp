...
void TCompLightSpot::registerMsgs() {
	...
	DECL_MSG(TCompLightSpot, TMsgSceneCreated, updateAABB);
...
void TCompLightSpot::updateAABB(const TMsgSceneCreated & msg) {
	...
	//		absAABB->Extents = extents;
	//		absAABB->Transform(*absAABB, posAABB.asMatrix());
	//	}

	//	//aabb.Transform(aabb, posAABB.asMatrix());
	//	//aabb.Center = VEC3(0, 0, 0);
	//	//aabb.Extents = extents;
	//	//renderWiredAABB(*absAABB, posAABB.asMatrix(), VEC4(0, 1, 0, 1));
	//}
	//else {
	//	VEC3 p = mypos->getPosition() + mypos->getFront() * (range / 2);
	//	posAABB.setPosition(p);
	...
	//		absAABB->Extents = VEC3(tan(deg2rad(angle / 2)) * range, range / 2, tan(deg2rad(angle / 2)) * range);
	//		absAABB->Transform(*absAABB, posAABB.asMatrix());
	//	}
	...
	//	aabb.Center = VEC3(0, 0, 0);
	//	aabb.Extents = VEC3(tan(deg2rad(angle / 2)) * range, range / 2, tan(deg2rad(angle / 2)) * range);*/
	//	//renderWiredAABB(*absAABB, posAABB.asMatrix(), VEC4(0, 0, 1, 1));
	//}
...
TCompLightSpot::result TCompLightSpot::createAABB() {
	...
	if (mycollider) {
		...
		res.extents = VEC3(tan(deg2rad(angle / 2)) * range, range / 2, tan(deg2rad(angle / 2)) * range);
		res.transform = posAABB;
		return res;
