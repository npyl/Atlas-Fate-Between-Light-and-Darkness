...
namespace Particles
{
	...
	bool CSystem::processParticle(TParticle& p, const VEC3& kWindVelocity, const float fadeRatio, const MAT44 world, const MAT44 world_rot, const float delta) {
	...
	void CSystem::updateSystem(float delta) {
		...
		if (_entity.isValid())
		{
			PROFILE_FUNCTION("Entity is valid");
			...
			if (EngineMT.isMultithreadingEnabled()) {
				size_t step = 1;
	...
	void CSystem::updateEmission(float delta) {
		PROFILE_FUNCTION("Update Emissions");
	...
	void CSystem::updateFading(float delta) {
		PROFILE_FUNCTION("Update Fading");
