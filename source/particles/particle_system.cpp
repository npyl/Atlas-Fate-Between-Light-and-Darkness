...
namespace Particles
{
	...
	//Processing each particle by separate to help threads do their job.
	//True if the particle is still alive, false if the particle dies.
	bool CSystem::processParticle(TParticle& p, const VEC3& kWindVelocity, float fadeRatio, float delta) {
		//TParticle &p = *it;
		p.lifetime += delta;
